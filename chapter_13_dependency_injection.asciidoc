...
We'll also add a new ((("bootstrapping")))
component to our architecture called __bootstrap.py__;
...
feeling of unease at the back of your mind at this point.((("dependency injection", "implicit versus explicit dependencies")))
Let's bring it out
...
all this is a bit weird.((("dependency injection", "explicit dependencies are better than implicit dependencies")))((("importing dependenies")))
The standard way to do things is to declare our
...
sake of our ((("mock.patch method")))
tests? `mock.patch` makes monkeypatching nice and easy:
...
On top of that, declaring((("dependency inversion principle", "declaring explicit dependency as example of")))((("abstractions", "explicit dependencies are more abstract")))
an explicit dependency is an example of the
...
Instead, we'll((("composition root")))((("bootstrapping", "dependency injection with")))
reach for a pattern called _Composition Root_ (a bootstrap
...
One way to turn((("dependency injection", "manual DI with closures or partial functions")))
a function with dependencies into one that's ready to be
called later with those dependencies _already injected_ is to use closures or
partial functions((("closures", "dependency injection using")))((("partial functions", "dependency injection with")))
to compose the function with its dependencies:
...
<1> The difference((("closures", "difference from partial functions")))((("partial functions", "difference from closures")))
    between closures (lambdas or named functions) and
...
of functional programming.((("dependency injection", "using classes")))((("classes, dependency injection using")))
Here's an alternative using classes, which may
...
We want our bootstrap((("bootstrapping", "bootstrapping script, capabilities of")))
...
Here's how we ((("dependency injection", "by inspecting function signatures")))
inject dependencies into a handler function by inspecting
it:
...
or if ((("dependency chains")))((("dependency injection", "using DI framework")))
you ever get into _dependency chains_ (in which your dependencies have their
...
handlers given to it.((("message bus", "class given handlers at runtime")))
So we turn it from being a module into a configurable
class:
...
What else changes ((("handlers", "event and command handlers in message bus")))((("message bus", "event and command handler logic staying the same")))((("commands", "command handler logic in message bus")))((("event handlers", "in message bus")))
in the bus?
...
In our application's entrypoints, we now((("bootstrapping", "using in entrypoints")))((("Flask framework", "calling bootstrap in entrypoints")))
just call `bootstrap.bootstrap()`
...
custom message bus.((("message bus", "getting custom with overridden bootstrap defaults")))((("bootstrapping", "initializing dependency injection in tests")))((("testing", "integration test for overriding bootstrap defaults")))
...
In our unit tests, in ((("testing", "unit test for bootstrap")))
contrast, we can reuse our `FakeUnitOfWork`:
...
To really get a feel((("adapters", "building adapter and doing dependency injection for it", id="ix_adapDI")))
for how it all works, let's work through an example of how
...
We'll imagine a more generic notifications API.((("adapters", "building adapter and doing dependency injection for it", "defining abstract and concrete implementations")))((("abstract base classes (ABCs)", "defining for notifications")))
Could be
...
We change the dependency((("bootstrapping", "changing notifications dependency in bootstrap script")))
...
We work through and define((("faking", "FakeNotifications for unit testing")))
a fake version for unit testing:
...
real-ish email((("Docker dev environment with real fake email server")))
server for our Docker dev environment:
...
In our integration((("bootstrapping", "using to build message bus that talks to real notification class")))
tests, we use the real `EmailNotifications` class,
...
You could do two things ((("adapters", "exercise for the reader")))
for practice regarding adapters:
...
* Once you have more than one adapter, you'll start to feel a lot of pain
  from passing dependencies around((("bootstrapping", "dependency injection and bootstrap recap")))((("dependency injection", "recap of DI and bootstrap")))
  manually, unless you do some kind of
