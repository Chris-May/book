...
The first thing is to choose where to install it.((("Django", "installing")))
We put it in a separate
...
Those custom methods ((("object-relational mappers (ORMs)", "Django, custom methods to translate to/from domain model")))((("domain model", "Django custom ORM methods for conversion")))
...
The tests((("Unit of Work pattern", "with Django", id="ix_UoWDjango")))((("Django", "Unit of Work pattern with", id="ix_DjangoUoW")))
...
The Django _views.py_ file ends ((("views", "Django views as adapters")))((("adapters", "Django views")))((("Django", "views are adapters")))((("APIs", "Django views as adapters")))
up being almost identical to the
...
to a Django app?((("Django", "applying patterns to Django app")))
...
term and if you want to migrate to some of our patterns later.((("Django", "applying patterns to Django app", "steps along the way")))
...
For more ((("Django", startref="ix_Django")))
thoughts and actual lived experience dealing with existing
