...
a system to another.((("events", "commands versus", id="ix_evntcmd")))((("commands", "events versus", id="ix_cmdevnt")))
We usually represent commands with dumb data
...
What kinds of commands((("commands", "events versus", startref="ix_cmdevnt")))((("commands", "in our system now")))
do we have in our system right now?
...
Just changing ((("events", "commands versus", startref="ix_evntcmd")))
the names and verbs is all very well, but that won't
change the behavior of our system. ((("exception handling, differences for events and commands")))
We want to treat events and commands similarly,
but not exactly the same.((("message bus", "dispatching events and commands differently")))
...
And here's((("commands", "exception handling")))
how we do commands:
...
commands and events. ((("dictionaries", "HANDLERS dicts for commands and events")))((("handlers", "new HANDLERS dicts for commands and events")))((("commands", "handlers for")))
Commands can have only one handler, according
to our convention:
...
event fails to process? ((("events", "events, commands, and error handling", id="ix_evntcmderr")))((("error handling", "events, commands, and", id="ix_errhnd")))((("commands", "events, commands, and error handling", id="ix_cmndeverr")))
How am I supposed to make sure the system is in a
...
happening.((("aggregates", "acting as consistency boundaries")))((("consistency boundaries", "aggregates acting as")))
We've carefully identified _aggregates_ that act as consistency
...
`Product` aggregate.((("Product object", "acting as consistency boundary")))
This means that we can't accidentally overallocate: either
...
events when rules are met.((("aggregates", "History aggregate recording orders and raising domain events")))
...
persist our state to the database.((("aggregates", "raising events about")))
What if we raised those events _before_ we
...
from the commands that raised them.((("errors, recovering from synchronously")))((("commands", "events, commands, and error handling", "recovering from errors synchronously")))
What should we do, then, to make sure we
...
Let's look again at the `handle_event` method ((("message bus", "handle_event method")))
from our message bus:
...
proverb ((("message bus", "handle_event with retries")))((("retries", "message bus handle_event with")))
says, "If at first you don't succeed, retry the operation with an
...
<1> Tenacity is a Python library ((("Tenacity library")))((("retries", "Tenacity library for")))
that implements common patterns for retrying.
...
of commands, but other guides often do it the other way around.((("commands", "splitting commands and events, trade-offs")))((("events", "splitting command and events, trade-offs")))
Making
