...
easily have solved with Django.((("events and the message bus", id="ix_evntMB")))
You might be asking if the increased testability
...
how the Unit of Work pattern((("Unit of Work pattern", "modifying to connect domain events and message bus")))((("events and the message bus", "events flowing through the system")))
can be modified to connect the two together elegantly,
...
So. Email alerts when we run out of stock. ((("email alerts, sending when out of stock", id="ix_email")))((("events and the message bus", "sending email alerts when out of stock", id="ix_evntMBeml")))
When we have new requirements like ones that _really_ have nothing to do with the core domain, it's all too easy to
...
As a one-off ((("events and the message bus", "sending email alerts when out of stock", "avoiding messing up web controllers")))
hack, this _might_ be OK:
...
we want them to be as ((("events and the message bus", "sending email alerts when out of stock", "avoiding messing up domain model")))((("domain model", "email sending code in, avoiding")))
thin as possible, we may look at putting it right at
...
Really, this is a violation ((("events and the message bus", "sending email alerts when out of stock", "violating the single responsibility principle")))((("single responsibility principle (SRP)")))
of the __single responsibility principle__ (SRP).footnote:[
...
flow control ((("orchestration", "changing to choreography")))((("choreography")))
changes what used to be _orchestration_ into _choreography_.] The
...
recording _events_—facts about things that have happened.((("events and the message bus", "recording events")))
We'll use a message bus to respond to events and invoke a new operation.

==== Events Are Simple Dataclasses

An _event_ is a kind of _value object_.((("events and the message bus", "events as simple dataclasses")))((("dataclasses", "events")))
Events don't have any behavior, because
...
but it can't, it ((("aggregates", "testing Product object to raise events")))
should _raise_ an event:
...
NOTE: We're actually addressing a ((("exceptions", "using for control flow")))((("control flow, using exceptions for")))
code smell we had until now, which is that we were
...
A message bus ((("events and the message bus", "message bus mapping events to handlers")))((("message bus", "mapping events to handlers")))
basically says, "When I see this event, I should invoke the following
handler function." In other words, it's a simple publish-subscribe system.
Handlers are _subscribed_ to receive events, which we publish to the bus.((("publish-subscribe system", "message bus as", "handlers subscribed to receive events")))
It
...
chunks of work to an asynchronous task queue.((("message bus", "Celery and")))((("Celery tool")))
The message bus we're
...
use _external events_ for that.((("external events")))
There's more discussion in
...
handlers whenever an event happens.((("domain model", "events from, passing to message bus in service layer")))((("message bus", "service layer with explicit message bus")))((("events and the message bus", "service layer with explicit message bus")))((("service layer", "taking events from model and putting them on message bus")))
Now all we need is to connect the two. We
...
in charge of creating and ((("service layer", "raising its own events")))((("events and the message bus", "service layer raising its own events")))((("message bus", "service layer raising events and calling messagebus.handle")))
raising events directly, rather than having them
...
currently in play because it provides access to the repository.((("events and the message bus", "UoW publishes events to message bus")))((("Unit of Work pattern", "UoW publishing events to message bus")))((("message bus", "Unit of Work publishing events to")))
So it's
...
<3> That relies on the repository ((("aggregates", "repository keeping track of aggregates passing through it")))((("repositories", "repository keeping track of aggregates passing through it")))
    keeping track of aggregates that have been loaded
...
<1> For the UoW to be able to publish new events, it needs to be able to ask
    ...
    We use a `set` called `.seen` to store them.((("super function")))
    That means our implementations
...
totally ((("service layer", "totally free of event handling concerns")))
...
We do also have to remember to change the fakes ((("underscorey methods", "tweaking fakes in service layer to implement")))((("faking", "tweaking fakes in service layer to call super and implement underscorey methods")))((("service layer", "tweaking fakes in to call super and implement underscorey methods")))((("super function", "tweaking fakes in service layer to call")))
in the service layer and make them
...
the words of our beloved tech reviewer Hynek? ((("underscorey methods", "avoiding by implementing TrackingRepository wrapper class")))
Does it "make you want to beat
...
order to get rid of those Java-y `_commit()` methods too.((("Unit of Work pattern", "getting rid of underscorey methods in UoW class")))
You can find the code on https://github.com/cosmicpython/code/tree/chapter_08_events_and_message_bus_exercise[GitHub].

Switching all ((("abstract base classes (ABCs)", "switching to typing.Protocol")))
the ABCs to `typing.Protocol` is a good way to force yourself to avoid using inheritance. Let us know if you come up with something nice!
******************************************************************************
...
And <<chapter_08_events_and_message_bus_tradeoffs>> shows ((("events and the message bus", "pros and cons or trade-offs")))((("message bus", "pros and cons or trade-offs")))
the trade-offs as we
see them.
...
* You also open yourself up to((("dependencies", "circular dependencies between event handlers")))
the possibility of circular dependencies between your
...
A message bus routes messages to handlers::
    ...
    consumers.((("message bus", "recap")))
    It doesn't "know" anything about the meaning of events; it's just
...
Option 2: Domain model raises events, service layer passes them to message bus::
    The logic about ((("domain model", "raising events and service layer passing them to message bus")))
    when to raise an event really should live with the model, so
...
Option 3: UoW collects events from aggregates and passes them to message bus::
    Adding `bus.handle(aggregate.events)` to every ((("Unit of Work pattern", "UoW collecting events from aggregates and passing them to message bus")))((("aggregates", "UoW collecting events from and passing them to message bus")))
    handler is annoying, so we
